...
- 托盘或菜单栏常驻，点击托盘图标显示或隐藏主窗口。右键菜单提供偏好设置、分隔线与退出；语言保存后更新已有菜单项，不重建 status item。
...
- 常用历史可置顶，按最近置顶时间排列在普通历史之前；置顶不占普通历史上限或主界面/分组条数，新增置顶上限默认 10 条，可在历史设置中调整为 5..=20；降低上限保留已有置顶，重复复制保留置顶时间。
...
- Preferences 使用可搜索的设置中心，包含行为、外观、历史、隐私、文本操作和 Agent CLI 六个页面；侧栏上层分组显示“通用”。语言、主题、图标、Logo 与序号归外观，保存类型、保留上限与两个列表展示条数归历史。控件立即保存，串行提交并处理失败回滚；保留紧凑开关、键盘焦点和禁用态。
- 文本 JSON/Base64/URL-component 快速操作按类型分行，结果在独立 `quick-action` 窗口展示；复制为主操作并写系统剪贴板，替换历史必须确认。详情高度按启用类型预留空间；关闭预览必须使未完成的转换请求失效。
...
src/
  ...
  components/QuickActionWindow.tsx    文本变换结果、复制与确认替换的独立窗口
  components/TextQuickActions.tsx     文本详情的有界快速操作入口
  components/HistoryDetailPanel.tsx   详情页的三段式外壳
  components/HistoryDetailDeleteButton.tsx
                                      主列表详情/分组详情复用的删除动作
  components/PinToastHost.tsx         主窗口/viewer 共用置顶错误 Toast，preview 定向发送安全错误，不抢焦点
  hooks/usePinHistory.ts              置顶 pending 与失效请求生命周期
  utils/pinHistory.ts                 结构化错误收窄、数字选择与可测试的置顶控制器
...
- `preview`、`preview-detail`、`about`、`preferences`、`image-viewer`、`quick-action` 都可能是首次使用时才创建；发 payload 或 show 前必须经过 `ensure_auxiliary_window` 与 ready 确认。
...
- `quick-action`：文本变换的独立固定窗口，560×420，可聚焦；结果默认仅存在内存中，复制写系统剪贴板，替换必须确认。
- `preferences`：偏好设置窗口，固定尺寸，默认隐藏。

辅助窗口创建由 `AuxiliaryWindowRegistry` 对并发请求去重，并等待前端 listener ready 后再允许调用方发送首个 payload。不要退回“七个 WebView 全部随进程启动”的配置。
...
- 正常历史变更通过带 revision 的 delta 定向发送；不要恢复成向所有窗口广播完整历史数组。外部文件 reconciliation 等异常路径仍可使用 typed `replace`。
...
- 字段包括 `launchAtLogin`、`language`、`menuBarIconStyle`、`autoPaste`、`maxHistoryCount`、`maxPinnedItems`、`enabledHistoryTypes`、`mainWindowItemCount`、`historyGroupItemCount`、`showHistoryItemNumbers`、`showMainWindowBrand`、`appearanceTheme`、`maskSensitiveContent`、`ignoredSourceAppIds`、`textQuickActions`。后者包含 `json`、`base64`、`urlComponent` 三个开关。
