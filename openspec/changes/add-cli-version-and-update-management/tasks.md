...
- [x] 7.6 For the eventual containing release, verify authenticated draft CLI/checksum downloads and digest validation on macOS and Windows before publication; record that draft verification does not constitute public-installer availability until the Release is published.
  - Release workflow run [35414182138](https://github.com/bells/mclip/actions/runs/35414182138) downloaded the macOS and Windows CLI/checksum pairs from the same `v0.2.0` Release, verified SHA-256, and executed `--version` before the release owner published the Draft. Public-installer smoke remains a separate open task in `prepare-v0-2-0-release`.
