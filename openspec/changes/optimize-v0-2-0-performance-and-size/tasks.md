...
## 5. 运行时与七窗口响应（2026-09-13 扩展）

- [x] 5.1 将普通记录上限同步为 1000（默认 200），补 Rust/TS 边界、置顶保留和产品文档/delta。
- [x] 5.2 审计启动关键路径、轮询、仓储、图片与七窗口；记录候选、现有措施和验证缺口。
- [x] 5.3 以 10/50/500/1000 条合成数据建立初次快照、热快照、单条查找、遮罩的分配和耗时基线，实施可证明的优化并对比。
- [x] 5.4 减少 macOS 空闲轮询设置复制，保留 changeCount、settle delay、错误回退和忽略来源语义。
- [x] 5.5 扩展 fixture 与真实组件 profiling 至 1000 条，检验搜索/选择、pointer/focus/Enter 与 DOM/高度。
- [x] 5.6 为七窗口与 Preferences 六页面补统一的首次/重复打开、页内操作、CPU/RSS 测量协议；运行自动化门禁并汇总本轮证据。
- [ ] 5.7 在 macOS release 上执行协议：至少 5 次进程启动，首次窗口单列、预热 5 次后交互 20 次；记录七窗口可见绘制和页内响应 median/p95、idle/active CPU、可归属 WebView RSS，核查 pointer/focus/主题/即时保存。
- [ ] 5.8 按 Windows 与 Linux 命名会话补同口径原生性能结果；不从 macOS 或 browser mock 推导完成。

2026-09-13 本轮完成 5.1–5.6；5.7 已取得 1000 条 fixture 的 5 次原生启动和 20 次 main/详情/viewer 热路径数据，但七窗口首用/页内操作与 CPU/RSS 尚缺，保持未勾选。详细边界见 [运行时报告](../../../performance/v0.2.0-runtime-response.md)。

2026-09-15 补充：完成 About/Preferences shell、六设置页、设置搜索/保存和 JSON quick-action 的首次及 20 次原生热响应；保存全量快照请求每次 1→0，时延改善低于 15% 目标，按报告说明。补采两段 60 秒 fixture 主进程 CPU/RSS。5.7 仍未勾选：分组原生 pointer、WebKit 归属、完整压力/主题/失败矩阵等尚缺，不用部分数据替代全部验收。
