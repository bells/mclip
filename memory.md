...
- Pins are ordered by most recent pin time before ordinary history, with a separate admission limit of 10 by default, configurable from 5 to 20 (2026-09-08). Lowering the limit preserves existing pins, including legacy collections up to 100. They do not consume ordinary retention/main/archive counts; dedupe preserves pin time. CLI supports `pin/unpin`, `--pinned`, and `clear --yes --keep-pinned`.
